...
function WorkProcess() {
  return (
    <div className='mx-auto mb-24 flex  max-w-large flex-col items-center justify-center'>
      ...
      <div className='grid grid-cols-12 gap-x-7 '>
        {WorkProcessCardList.map((item, index) => {
          ...
          return (
            <div
              className='relative col-span-4 flex flex-col gap-y-5 bg-gray-100 px-10 py-14 text-center'
              key={index}
            >
              {/*Icons*/}
              <div className='flex-center absolute left-[50%] top-[-10%] translate-x-[-50%] translate-y-[-10%]'>
                <Image
                  src={icon}
                  alt={title}
                  width={72}
